...
def main():
  ...
  encoder, discriminator_class, discriminator_domain = split_model(model)
  if config.GPU and torch.cuda.is_available():
  ...
  optimizer_class = optim.SGD(discriminator_class.parameters(), 
                        lr=config.LEARNING_RATE,
                        momentum=config.MOMENTUM)
  optimizer_domain = optim.SGD(discriminator_domain.parameters(), 
                        lr=config.LEARNING_RATE,
                        momentum=config.MOMENTUM)
  explorer = lr_scheduler.StepLR(optimizer_encoder, 
